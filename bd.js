...
(function ($jquery, win, prefix) {
    ...
    var classes_init = ['bdjs'], classes = classes_init;
    var tests = [];
    var feature_detect = {
    ...
    var detect_device_type = function () {
        ...
        if (os['ios']) {
            ...
            if (ua(reg_iosv)) {
                //only report the iOS version if we can understand the string
            ...
            var reg_andr = /(?:Linux; Android|Linux; U; Android|Android)[\s]([\d]).([\d])(?:.([\d])+)*;/i;
            if (ua(reg_andr)) {
